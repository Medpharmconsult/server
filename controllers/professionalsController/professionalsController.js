...
professionalsController.setSchedule = ("/set-schedule", async (req, res)=>{
    try {
        ...
        if(payload.scheduleType ==  "range"){
            ...
            for(let d = startDayInt; d <= endDayInt ; d++){
                for(let t = startTimeInt; t <endTimeInt; t++){  
                    ...
                    const existingSchedule = await database.findOne({year: schedule.year, month: schedule.month, day: schedule.day, startTime: schedule.startTime, endTime: schedule.endTime}, database.collection.schedules)
                    if(existingSchedule){
                        utilities.setResponseData(res, 400, {'content-type': 'application/json'}, {statusCode: 400, responseData: {msg: "schedule already exists"}}, true)
                        return
                    }
        ...
        if(payload.scheduleType ==  "single"){
            //Break down data
            const schedules = []
            const dayInt = parseInt(payload.day)
            const startTimeInt = timeMap[`${payload.startTime}${payload.startMeridian}`]
            const endTimeInt = timeMap[`${payload.endTime}${payload.endMeridian}`]
